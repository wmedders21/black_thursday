...
class SalesEngine
  ...
  def invoice_items
    iir_array = @table_hash[:invoice_items].map do |row|
      InvoiceItem.new({ id: row[:id].to_i, item_id: row[:item_id].to_i, invoice_id: row[:invoice_id].to_i,
                        quantity: BigDecimal(row[:quantity]), unit_price: BigDecimal(row[:unit_price], row[:unit_price].length)/100, created_at: Time.parse(row[:created_at]), updated_at: Time.parse(row[:updated_at]) })
    end
  ...
  def transactions
    transaction_array = @table_hash[:transactions].map do |row|
      Transaction.new ({id: row[:id].to_i, invoice_id: row[:invoice_id].to_i, credit_card_number: row[:credit_card_number], credit_card_expiration_date: row[:credit_card_expiration_date], result: row[:result].downcase.to_sym, created_at: row[:created_at], updated_at: row[:updated_at]})
    end
